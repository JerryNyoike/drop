...
@bp.route('fetch/in', methods=['POST'])
def fetch_in():
    if request.content_type != 'application/json':
    ...
    beat INNER JOIN producer ON beat.producer_id=producer.producer_id WHERE beat.beat_id IN ({})'''.format(','.join(beat_ids))
