    ...
    if request.content_type == 'application/json':
        ...
        if not user_exists(request_data['type'], request_data['email']):
            ...
            if result is not None:
                return make_response({'status': 1, 'message': 'Registration successfull'})
            else:
                print(result)
                return make_response({'status': 0, 'message': 'Registration unsuccessfull'})
        else:
...
def login():
    ...
    if request.content_type == 'application/json':
        ...
        if user_data != None:
            ...
            token = jwt.encode({'aud': request_data['type'], 'exp': datetime.now() + timedelta(days=10)}, current_app.config['SCRT'], algorithm='HS256').decode('utf-8')
...
def fetch_user(request_data):
    ...
    query = "SELECT * FROM {} WHERE `email` = '{request_data['email']}' AND `pwd` = '{request_data['pwd']}' LIMIT 1".format(table)
    cur.execute(query)
    return cur.fetchone()
...
def is_logged_in(token):
    try:
        ...
        return None
    except jwt.exceptions.ExpiredSignatureEror as e:
        return False
